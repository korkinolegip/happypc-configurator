...
const BuildCard: React.FC<BuildCardProps> = ({ build }) => {
  return (
    <div className="bg-[#111111] border border-[#2A2A2A] rounded-lg p-4 hover:border-[#FF6B00]/40 transition-colors">
      {/* Title + sum */}
      <div className="flex items-start justify-between gap-3 mb-2">
        <Link
          to={`/b/${build.short_code}`}
          className="text-white font-semibold text-base hover:text-[#FF6B00] transition-colors leading-tight line-clamp-2 flex-1"
        >
          {build.title}
        </Link>
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[#888888] text-xs">{build.author_name}</span>
        {build.workshop_name && (
          <span className="text-[#555555] text-xs truncate">• {build.workshop_name}</span>
        )}
        <span className="text-[#444444] text-xs ml-auto shrink-0">{formatDate(build.created_at)}</span>
      </div>
      ...
      <div className="flex items-center gap-3 pt-2 border-t border-[#1E1E1E]">
        <button className="flex items-center gap-1 text-[#444444] hover:text-[#FF6B00] transition-colors text-xs">
          <ThumbsUp size={13} /><span>0</span>
        </button>
        <button className="flex items-center gap-1 text-[#444444] hover:text-[#AAAAAA] transition-colors text-xs">
          <MessageSquare size={13} /><span>0</span>
        </button>
        <button className="flex items-center gap-1 text-[#444444] hover:text-red-400 transition-colors text-xs">
          <Heart size={13} /><span>0</span>
        </button>
        <button className="flex items-center gap-1 text-[#444444] hover:text-[#AAAAAA] transition-colors text-xs ml-auto">
          <Share2 size={13} />
        </button>
      </div>
    </div>
